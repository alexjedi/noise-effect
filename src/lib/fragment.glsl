...
void main() {
    ...
    if (VAR == 0) {
        ...
        sdf = sdCircle(st, vec2(0.5));
        sdf = stroke(sdf, 0.5, 0.4, sdfCircle) * 6.0;
    } else if (VAR == 3) {
        /* sdf circle with fill param adjusted by sdf circle */
    ...
    // Calculate gradient based on distance to center
    float dist = length(st - vec2(0.5));
    vec3 gradientColor = mix(vec3(0.0), vec3(1.0), smoothstep(0.0, 0.5, dist));

    vec3 color = gradientColor * vec3(sdf);
